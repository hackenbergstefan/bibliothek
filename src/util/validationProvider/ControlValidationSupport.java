package util.validationProvider;
 ...
 * @since 1.3
 */
public class ControlValidationSupport {
	/**
	 ...
	 */
	public static ControlValidationSupport create(Control[] controls,
			DataBindingContext dbc) {
		return new ControlValidationSupport(controls, dbc);
	}
...
//	private IValidationMessageProvider messageProvider = new ValidationMessageProvider();
	...
	private ControlValidationSupport(Control[] controls,
			DataBindingContext dbc) {
...
//	}
	...
	private void init() {
		...
		controls[0].getShell().addDisposeListener(new DisposeListener() {
	...
	private void handleStatusChanged() {
		if (controls[0] == null || controls[0].getShell() == null || controls[0].getShell().isDisposed())
			return;
...
//		}
		for(Control c: controls) c.setEnabled(currentStatus == null?true:currentStatus.isOK());
	}
